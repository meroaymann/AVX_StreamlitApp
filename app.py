...
def _find_logo_data_uri() -> Optional[str]:
    # Put your logo image next to app.py with any of these names
    candidates = [
        "logo.png",
        "logo.webp",
        "logo.jpg",
    ...
    for fn in candidates:
        if os.path.exists(fn):
            uri = _file_to_data_uri(fn)
            if uri:
                return uri
    return None


def inject_pro_ui(logo_data_uri: Optional[str]) -> None:
    bg_logo = logo_data_uri or ""
    ...
    st.markdown(
        ...
        .stApp {{
          background:
            radial-gradient(1200px 700px at 10% 5%, rgba(78, 99, 255, 0.18), transparent 55%),
            radial-gradient(900px 550px at 95% 12%, rgba(255, 122, 0, 0.14), transparent 55%),
            linear-gradient(180deg, rgba(16, 18, 27, 0.02), rgba(16, 18, 27, 0.00));
        }}
        ...
        .topbar {{
          ...
          position: relative;
          overflow: hidden;
          animation: fadeInDown 520ms ease-out;
        }}
        .topbarGlow {{
          position: absolute;
          width: 380px;
          height: 380px;
          border-radius: 999px;
          background: radial-gradient(circle, rgba(78,99,255,0.28), transparent 62%);
          top: -240px;
          left: -180px;
          opacity: 0.85;
        ...
        .brandTitle {{ font-weight: 900; font-size: 18px; margin: 0; line-height: 1.2; }}
        .brandSub {{ margin: 0; opacity: 0.78; font-size: 12.5px; }}
        .crumb {{
          font-size: 12px;
          padding: 8px 10px;
          border-radius: 999px;
        ...
        .hero {{
          ...
          padding: 22px 22px;
          background:
            linear-gradient(135deg, rgba(78,99,255,0.18), rgba(0,0,0,0.00)),
            linear-gradient(45deg, rgba(255,122,0,0.12), rgba(0,0,0,0.00));
        ...
        .heroGlow {{
          ...
          right: -260px;
          opacity: 0.8;
          animation: floaty 6.0s ease-in-out infinite;
        }}
        .heroTitle {{ font-size: 34px; font-weight: 950; margin: 0 0 6px 0; }}
        .heroSub {{ font-size: 14.5px; opacity: 0.86; margin: 0; line-height: 1.7; }}
        ...
        .cardGrid {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin-top: 14px; }}
        .card {{
          border-radius: 18px;
          padding: 16px 16px;
          border: 1px solid rgba(0,0,0,0.10);
        ...
        .card h4 {{ margin: 0 0 6px 0; font-weight: 900; }}
        .card p {{ margin: 0; opacity: 0.86; line-height: 1.6; font-size: 13.5px; }}

        .softDivider {{ height: 1px; background: linear-gradient(90deg, transparent, rgba(0,0,0,0.12), transparent); margin: 16px 0; }}
        ...
        .floatNav {{
          ...
          backdrop-filter: blur(10px);
          animation: fadeInUp 540ms ease-out;
        }}
        .floatHint {{ font-size: 12px; opacity: 0.80; padding: 8px 10px; border-radius: 12px; background: rgba(0,0,0,0.03); }}

        /* =========================
           LANDING LOGO (centered, smaller, cropped, no frame)
           ========================= */
        .landingWrap {{
          display: flex;
        ...
        /* Style ONLY buttons inside landing container */
        .landingStart div[data-testid="stButton"] > button {{
        ...
        .landingStart div[data-testid="stButton"] > button:hover {{
          transform: translateY(-1px);
          transition: transform 120ms ease;
          filter: brightness(1.02);
        }}
        .landingStart div[data-testid="stButton"] > button:focus {{
          box-shadow: none !important;
        }}
        .landingSub {{
          opacity: 0.78;
        ...
        @keyframes fadeInUp {{ from {{ transform: translateY(10px); opacity: 0; }} to {{ transform: translateY(0px); opacity: 1; }} }}
        @keyframes fadeInDown {{ from {{ transform: translateY(-10px); opacity: 0; }} to {{ transform: translateY(0px); opacity: 1; }} }}
        @keyframes floaty {{ 0% {{ transform: translate(0px,0px);}} 50% {{ transform: translate(16px,10px);}} 100% {{ transform: translate(0px,0px);}} }}
...
def topbar(page_label: str) -> None:
    st.markdown(
        f"""
        <div class="topbar">
          <div class="topbarGlow"></div>
          <div class="brandLeft">
            <div>{app_logo_svg(44)}</div>
            <div>
              <div class="brandTitle">ArabicX-Viz</div>
              <div class="brandSub">Arabic VQA + Visual Explainability | Thesis demo</div>
            </div>
          </div>
          <div class="crumb">You are here: <b>{page_label}</b></div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def floating_nav() -> None:
    st.markdown(
        """
        <div class="floatNavWrap">
          <div class="floatNav">
            <div class="floatHint">Quick navigation: Home | System | How to Test | Demo</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
...
# 2) Navigation
# ============================================================
PAGES = ["Home", "About the System", "How to Test", "Try the System"]
...
# ============================================================
# 12) XAI: Example-based explanations (retrieval) aligned to BLIP
# ============================================================
...
@torch.no_grad()
def blip_embed_image(pil_img: Image.Image) -> np.ndarray:
    vqa_model.eval()
    inputs = vqa_proc(images=pil_img, return_tensors="pt").to(device)
    vision_out = vqa_model.vision_model(pixel_values=inputs["pixel_values"], return_dict=True)
    hs = vision_out.last_hidden_state
    cls = hs[:, 0, :]
    vec = cls[0].detach().float().cpu().numpy()
    return _l2_normalize(vec)


@torch.no_grad()
def blip_embed_text(question_en: str) -> np.ndarray:
    q = (question_en or "").strip()
    if not q:
        return np.zeros((768,), dtype=np.float32)
    vqa_model.eval()
    t_inputs = vqa_proc(text=q, return_tensors="pt").to(device)
    te = vqa_model.text_encoder(
        input_ids=t_inputs.get("input_ids", None),
        attention_mask=t_inputs.get("attention_mask", None),
        return_dict=True,
        output_attentions=False,
    )
    hs = te.last_hidden_state
    cls = hs[:, 0, :]
    vec = cls[0].detach().float().cpu().numpy()
    return _l2_normalize(vec)
...
@torch.no_grad()
def blip_joint_query_embedding(pil_img: Image.Image, question_en: str) -> np.ndarray:
    img_v = blip_embed_image(pil_img)
    txt_v = blip_embed_text(question_en)
    joint = np.concatenate([img_v, txt_v], axis=0).astype(np.float32)
...
def init_retrieval_memory() -> None:
    st.session_state.setdefault("retrieval_gallery", [])
    st.session_state.setdefault("retrieval_embed_cache", {})


def _example_key(img_key: str, question_en: str) -> str:
    q = (question_en or "").strip().lower()
    return f"{img_key}||{q}"


def add_example_to_gallery(
    pil_img: Image.Image,
    img_key: str,
    question_en: str,
    answer_en: str,
    ...
    k = _example_key(img_key, question_en)
    for ex in st.session_state.retrieval_gallery:
        if ex.get("key") == k:
            return False

    if k in st.session_state.retrieval_embed_cache:
        emb = st.session_state.retrieval_embed_cache[k]
    else:
        emb = blip_joint_query_embedding(pil_img, question_en)
        st.session_state.retrieval_embed_cache[k] = emb

    st.session_state.retrieval_gallery.append(
        {
            "key": k,
            "img_key": img_key,
            "image": pil_img.copy(),
            ...
            "answer_ar": (answer_ar or "").strip(),
            "caption_en": (caption_en_text or "").strip(),
            "caption_ar": (caption_ar_text or "").strip(),
            "embedding": emb,
        }
    )
    return True
...
def retrieve_similar_examples(
    ...
    init_retrieval_memory()
    gallery = st.session_state.retrieval_gallery
    if not gallery:
        return [], {"mode": "empty_gallery"}

    q_emb = blip_joint_query_embedding(pil_img, question_en)
    qk = _example_key(img_key, question_en)

    scored: List[Tuple[float, Dict[str, Any]]] = []
    for ex in gallery:
        ex_emb = ex.get("embedding", None)
        if ex_emb is None:
            continue
    ...
    top = [dict(ex, similarity=float(sim)) for sim, ex in scored[: max(1, int(topk))]]

    return top, {
        "mode": "ok",
        "gallery_size": int(len(gallery)),
        "topk": int(topk),
        "note": "Similarity is cosine on BLIP-based joint (image+question) embedding.",
    }
...
def build_pdf_report(
    ...
    a_ar: str,
    cap_ar: str,
    overlay_ans: Optional[Image.Image],
    overlay_attn: Optional[Image.Image],
) -> io.BytesIO:
    buffer = io.BytesIO()
    ...
    c.drawString(40, Hp - 50, "ArabicX-Viz | VQA + XAI Report")

    c.setFont("Helvetica", 11)
    c.drawString(40, Hp - 80, f"Q (AR): {q_ar}")
    c.drawString(40, Hp - 98, f"A (AR): {a_ar}")
    c.drawString(40, Hp - 116, f"Caption (AR): {cap_ar}")

    c.setFont("Helvetica-Bold", 12)
    c.drawString(40, Hp - 145, "Original Image")
    img_bytes = io.BytesIO()
    pil_img.save(img_bytes, format="PNG")
    img_bytes.seek(0)
    c.drawImage(ImageReader(img_bytes), 40, Hp - 470, width=320, height=320)
    c.showPage()

    if overlay_ans is not None:
        c.setFont("Helvetica-Bold", 12)
        c.drawString(40, Hp - 50, "Answer-aware Heatmap (Intervention-weighted)")
        b = io.BytesIO()
        overlay_ans.save(b, format="PNG")
        b.seek(0)
        c.drawImage(ImageReader(b), 40, Hp - 420, width=420, height=320)
        c.showPage()

    if overlay_attn is not None:
        c.setFont("Helvetica-Bold", 12)
        c.drawString(40, Hp - 50, "Spatial Attention (Question-guided Cross-Attention)")
        b = io.BytesIO()
        overlay_attn.save(b, format="PNG")
        b.seek(0)
        c.drawImage(ImageReader(b), 40, Hp - 420, width=420, height=320)
        c.setFont("Helvetica", 10)
        c.drawString(40, Hp - 440, "Note: ViT patch-grid attention (proxy from BLIP text encoder cross-attention).")
        c.showPage()
...
def page_home() -> None:
    ...
    if os.path.exists("GIU_logo.png"):
        st.image("GIU_logo.png", caption="University logo", width=180)


def page_about_system() -> None:
    topbar("About the System")
    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        <div class="aboutBar">
          <b>About</b>
          <span>This page is intentionally clean. Expand the boxes to read details.</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    hero(
        "About the System",
        "A single interactive pipeline: Arabic question → English proxy → BLIP VQA → Arabic answer, with visual explanations aligned to the image.",
        badges=["Translation: M2M100", "VQA: BLIP", "Caption: ViT-GPT2", "Seg: YOLOv8-seg", "Retrieval XAI", "Counterfactual XAI", "LIME (optional)"],
    )
    ...
    st.markdown(
        ...
        <div class="cardGrid">
          <div class="card">
            <h4>Input</h4>
            <p>Upload an image and ask a question in Arabic. The system translates it to English as a proxy for the VQA model.</p>
          </div>
          <div class="card">
            <h4>Core inference</h4>
            <p>BLIP generates an answer. A confidence proxy is computed from token probabilities of the generated output.</p>
          </div>
          <div class="card">
            <h4>Explainability</h4>
            <p>Multiple XAI views: object interventions, answer-aware heatmap, cross-attention, saliency, ViT Grad-CAM, retrieval explanations, counterfactual answer flips, plus optional <b>LIME</b>.</p>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    with st.expander("Models inside (exact names)", expanded=True):
        st.write(
            {
                "Caption (EN)": "nlpconnect/vit-gpt2-image-captioning",
                "VQA (EN)": "Salesforce/blip-vqa-base",
                "Translation (EN<->AR)": "facebook/m2m100_418M",
                "Segmentation": "yolov8n-seg.pt (Ultralytics)",
            }
        )
...
def page_demo() -> None:
    ...
    SECTIONS = [
        ...
        "10) Example-based explanations (Retrieval)",
        "11) LIME for VQA (superpixels, optional)",
        "12) SHAP for multimodal inputs (superpixels + token groups)",
        "13) PDF Report",
        "14) Evaluation",
    ]
    ...
    if section == SECTIONS[0]:
        ...
        st.subheader("📚 Example-based explanations (Retrieval)")
        st.caption("Retrieves similar (image + question) examples from your demo gallery using BLIP-based embeddings.")

        cA, cB, cC = st.columns([1.0, 1.0, 1.0], gap="large")
        with cA:
            topk = st.slider("Top-K similar examples", 1, 6, 4)
        with cB:
            auto_add = st.checkbox("Auto-add this QA to gallery", value=False)
        with cC:
            clear = st.button("🧹 Clear gallery", use_container_width=True)

        if clear:
            st.session_state.retrieval_gallery = []
            st.session_state.retrieval_embed_cache = {}
            st.success("Gallery cleared.")

        add_now = st.button("➕ Add current (image + question + model answer) to gallery", use_container_width=True)

        if auto_add or add_now:
            added = add_example_to_gallery(
                pil_img=pil_img,
                img_key=img_key,
                question_en=q_en,
                answer_en=base_ans_en,
                answer_ar=base_ans_ar,
                caption_en_text=cap_en,
                caption_ar_text=cap_ar,
            )
            if added:
                st.success("Added to gallery. Now retrieval has more examples.")
            else:
                st.info("This exact (image + question) is already in the gallery.")

        with st.spinner("Retrieving similar examples using BLIP embeddings..."):
            results, rdbg = retrieve_similar_examples(pil_img, img_key, q_en, topk=int(topk))

        if rdbg.get("mode") == "empty_gallery":
            st.warning("Gallery is empty. Add a few examples first using the button above.")
            st.markdown(
                """
                **Quick demo tip:** Add 3 to 8 examples with different images and questions.
                Then retrieval becomes convincing and helps debug mistakes.
                """
            )
            return

        st.markdown("#### 🔎 Current query")
        st.write({"question_en": q_en, "answer_en": base_ans_en, "answer_ar": base_ans_ar, "gallery_size": rdbg.get("gallery_size", 0)})
        ...
        if not results:
            ...
            for rank, ex in enumerate(results, start=1):
                ...
                with c1:
                    st.image(ex["image"], caption=f"Example #{rank} (similarity={sim:.3f})", use_container_width=True)
                with c2:
                    st.markdown(f"**Q (EN):** {ex.get('question_en','')}")
                    st.markdown(f"**A (EN):** {ex.get('answer_en','')}")
                    st.markdown(f"**A (AR):** {ex.get('answer_ar','')}")
                    if ex.get("caption_ar"):
                        st.markdown(f"**Caption (AR):** {ex.get('caption_ar','')}")
                    st.caption("Interpretation: if nearest examples look relevant but answers differ, this is a strong debugging signal.")
        ...
        if objs_sorted:
            heat_ans = answer_aware_heatmap_from_objects(objs_sorted, H, W)
            heat_ans_pp = postprocess_heatmap(heat_ans, blur_sigma=6.0, clip_percentile=99.0, gamma=0.7)
            overlay_ans = overlay_heatmap_jet(pil_img, heat_ans_pp, alpha=0.45)

        if attn_heat01 is not None:
            attn_pp = postprocess_heatmap(attn_heat01, blur_sigma=8.0, clip_percentile=99.0, gamma=0.7)
            overlay_attn = overlay_heatmap_jet(pil_img, attn_pp, alpha=0.45)

        pdf_buffer = build_pdf_report(pil_img, q_ar_clean, base_ans_ar, cap_ar, overlay_ans, overlay_attn)

        st.download_button(
            "⬇️ Download PDF report",
            data=pdf_buffer,
            file_name="ArabicXViz_report.pdf",
            mime="application/pdf",
            use_container_width=True,
        )

    elif section == SECTIONS[13]:
        st.subheader("📊 Evaluation (User Study)")
        st.caption("Manual evaluation for thesis reporting. You can screenshot or later save to CSV if you want.")

        user_conf = st.slider("ما مدى ثقتك في أن الإجابة العربية صحيحة؟", 1, 5, 3)
        xai_help = st.slider("ما مدى فائدة الشروحات البصرية؟", 1, 5, 3)
        ret_help = st.slider("ما مدى فائدة أمثلة الاسترجاع (Retrieval)؟", 1, 5, 3)
        cf_help = st.slider("ما مدى فائدة التفسيرات المضادة (Counterfactuals)؟", 1, 5, 3)
        lime_help = st.slider("ما مدى فائدة LIME (إن وُجد)؟", 1, 5, 3)

        st.write(
            {
                "question_ar": q_ar_clean,
                "question_en": q_en,
                "answer_en": base_ans_en,
                "answer_ar": base_ans_ar,
                "base_conf": round(base_conf, 4),
                "user_confidence_1_5": user_conf,
                "xai_helpfulness_1_5": xai_help,
                "retrieval_helpfulness_1_5": ret_help,
                "counterfactual_helpfulness_1_5": cf_help,
                "lime_helpfulness_1_5": lime_help,
                "retrieval_gallery_size": len(st.session_state.get("retrieval_gallery", [])),
            }
        )
...
# 15) App main
# ============================================================
logo_data_uri = _find_logo_data_uri()
st.session_state["_logo_data_uri"] = logo_data_uri

inject_pro_ui(logo_data_uri)

with st.sidebar:
    st.markdown("### Navigation")
    page_choice = st.radio("Go to", PAGES, index=PAGES.index(get_page()), label_visibility="collapsed")
    st.session_state.page = page_choice

    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    st.caption("ArabicX-Viz Demo UI | Streamlit")
    ...
    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    if st.button("⬆️ Scroll to top", use_container_width=True):
        scroll_to_top()

    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    st.markdown("### Retrieval gallery (demo index)")
    init_retrieval_memory()
    st.write({"gallery_size": len(st.session_state.get("retrieval_gallery", []))})
    st.caption("Add examples from the Demo page. This is the small curated index for retrieval XAI.")

    st.markdown('<div class="softDivider"></div>', unsafe_allow_html=True)
    st.markdown("### Optional: LIME status")
    st.write(
        {
            "lime_installed": lime_image is not None,
            "skimage_installed": slic is not None,
            "shap_installed": shap is not None,
        }
    )
